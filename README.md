...
| Данные | SQLite через `@libsql/client` (файл на диске; опционально Turso `libsql://`), аудио — на локальном диске/volume за абстракцией `AudioStorage` |
| Тесты | Vitest + Supertest (интеграционные тесты API), Playwright/Puppeteer-сценарий полного пути (см. «Проверка») |
| Деплой | один Node-процесс отдаёт и API, и собранный клиент; Dockerfile, `docker-compose.yml`, `fly.toml`, `railway.json`, `render.yaml`, GitHub Actions CI |
...
Клиенту переменные не нужны: он ходит на относительный `/api`.
...
- Сервер после каждой оценки пересчитывает средние по каждому критерию и `total = avgQuality + avgListenability + avgPersonal` (максимум 90) и возвращает обновлённый трек. Клиент никогда не вычисляет общий балл сам.
...
- `npm test` — 16 интеграционных тестов API: неверный/верный код, изоляция артистов по `artistKey`, публикация и валидация аудио, оценки (upsert, пересчёт, запрет артисту), удаление только владельцем, `410` для удалённого трека, восстановление гостя по коду.
...
- Все проверки прав — на сервере; клиентские проверки только для UX.
