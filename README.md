...
4. При старте приложение пробует `GET /api/health` на своём домене; не ответил — включает локальный движок. Хотите общую базу на том же Pages-ссылке: **Профиль → Источник данных → подключить API** (или `?api=https://…`), ничего пересобирать не нужно.
