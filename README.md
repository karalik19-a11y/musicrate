...
**Приложение открывается просто по ссылке с GitHub Pages** — [karalik19-a11y.github.io/musicrate](https://karalik19-a11y.github.io/musicrate/). Pages отдаёт статический клиент, но сам по себе не умеет хранить общие данные: опубликованный трек должен записываться в общий API. Публичный бандл уже настроен на `https://musicrate.onrender.com`, поэтому артист и гости с разных устройств видят одну ленту. Если API нужно разместить в другом месте, задай `MUSICRATE_API_URL` в переменных репозитория Pages или подключи адрес в профиле. Локальный режим остаётся явным офлайн-режимом для одного устройства (см. [Два источника данных](#два-источника-данных)).
...
- **Telegram-бот и Mini App**: приложение открывается внутри Telegram по кнопке бота, а каждый новый трек прилетает подписчикам сообщением с кнопкой «Открыть трек» — deep link ведёт сразу на страницу трека (подробнее в [Telegram-бот](#telegram-бот-и-mini-app)).
...
│  │  ├─ telegram/         # бот: long polling, /start → подписка, рассылка новых треков
...
| `TELEGRAM_BOT_TOKEN` | — | токен бота от @BotFather. Задан → сервер запускает бота: `/start` подписывает на рассылку, каждый новый трек приходит подписчикам с кнопкой «Открыть трек». Не задан → бота нет, остальное работает как раньше |
| `TELEGRAM_WEBAPP_URL` | `https://karalik19-a11y.github.io/musicrate/` | HTTPS-адрес веб-приложения, который открывают кнопки бота (Telegram запускает только HTTPS mini apps) |
| `TELEGRAM_API_URL` | `https://api.telegram.org` | origin Bot API — можно указать свой [local Bot API server](https://core.telegram.org/bots/api#using-a-local-bot-api-server) |
...
|---|---|---|
| **`http`** | есть `VITE_API_URL`, сохранённый адрес API или `/api/health` отвечает с того же домена | SQLite + диск на сервере (`server/`). Публичный рейтинг считается сервером |
| **`local`** | явно выбран через `?api=local`, либо сборка не настроена на API и same-origin `/api/health` недоступен | IndexedDB этого браузера: метаданные, Blob'ы аудио, сессии, оценки, tombstones удалённых треков |
...
Чего локальный режим **не** даёт: общая база между устройствами и настоящая защита кода артиста — статический хостинг физически не может ничего скрыть от пользователя. Поэтому публичный Pages-бандл настроен на общий API; локальный режим оставлен для офлайн/разработки и включается явно.
...
| Платформа | Как |
|---|---|
| **GitHub Pages (статический клиент)** | уже настроено: бандл лежит в `docs/`, Pages публикует `main` из корня, а `index.html` в корне перекидывает в `./docs/`. Бандл по умолчанию обращается к общему API `https://musicrate.onrender.com`, поэтому гости с разных устройств видят опубликованные треки. Для другого API задай переменную репозитория `MUSICRATE_API_URL`; без работающего API общая лента невозможна. Обновление — `npm run pages:build` и коммит; workflow [`pages.yml`](.github/workflows/pages.yml) делает это автоматически при пуше в `main`. |
...
4. Публичный Pages-бандл собирается с `VITE_API_URL=https://musicrate.onrender.com` (или со значением `MUSICRATE_API_URL` из переменных репозитория). Поэтому он не переключается незаметно на личный IndexedDB: если общий API временно недоступен, приложение показывает ошибку, а не принимает трек в хранилище только одного устройства. Для другого API можно подключить его через **Профиль → Источник данных** или `?api=https://…`; локальный режим включается явно через `?api=local`. На стороне API достаточно, чтобы `ALLOWED_ORIGINS` включал домен Pages (по умолчанию `*`) — preflight и `Range` для `<audio>` сервер отвечает сам.
...
Приложение работает как **Telegram Mini App**, а бот из того же репозитория рассылает уведомления о новых треках. Отдельный процесс не нужен: бот живёт внутри Node-сервера API (`server/src/telegram/`) и общается с Telegram **long polling'ом** — вебхуки и публичный URL не требуются, он работает даже с `localhost`.

| Событие | Что делает бот |
|---|---|
| `/start` | подписывает чат на рассылку (таблица `telegram_subscribers` в SQLite — переживает рестарт) и отвечает приветствием с кнопкой **«▶️ Открыть MUSICRATE»** (`web_app`), которая запускает приложение внутри Telegram |
| Артист публикует трек | все подписчики получают «🎧 Новый трек на MUSICRATE … название, автор, длительность» с кнопкой **«🎧 Открыть трек»** — deep link `…/#/track/<id>` открывает приложение сразу на странице этого трека |
| `/stop` | отключает рассылку (вернуть — снова `/start`) |
| Пользователь заблокировал бота | чат молча удаляется из подписчиков (`403` / `my_chat_member: kicked`) |
| Любое другое сообщение | короткая справка + кнопка открытия |

### Включение (2 шага)

1. Создай бота у [@BotFather](https://t.me/BotFather): `/newbot` → имя и username → получи токен вида `123456789:AA…`.
2. Задай его как `TELEGRAM_BOT_TOKEN` там, где работает API с общей базой (Render — Dashboard → Environment; Docker — `-e`/compose; локально — `.env`). Перезапусти сервис.

Всё: в логе появится `[telegram] bot @… is up`, кнопка меню чата становится кнопкой открытия приложения, а `/start` подписывает на рассылку. Без токена сервер работает ровно как раньше.

Необязательно, но удобно: у @BotFather командой `/setcommands` можно добавить подсказки `start — слушать новые треки` и `stop — отключить рассылку`, а `/setmenubutton` / «Bot Settings → Menu Button» — закрепить кнопку приложения (если автонастройка через Bot API не сработала).

### Что умеет Mini App (клиент)

`client/index.html` подключает официальный SDK `telegram-web-app.js`; логика — в `client/src/lib/telegram.ts`. Вне Telegram всё это — строгий no-op, сайт работает как раньше. Внутри Telegram:

- приложение раскрывается на весь экран, шапка и фон Telegram подстраиваются под `#060607` — интерфейс выглядит нативным;
- вертикальный свайп не закрывает приложение во время скролла, а при играющем треке закрытие спрашивает подтверждение;
- нативная кнопка **← Назад** в шапке повторяет навигацию приложения (на корневых табах и по deep-link без истории — прячется);
- тактильная отдача (haptics) оценок и нажатий идёт через Telegram;
- поле имени гостя предзаполняется именем из Telegram.

### Если бот молчит или не открывается

Диагностика одной строкой — открой `https://<твой-сервер>/api/telegram/status`:

```json
{ "enabled": true, "running": true, "bot": "@musicrate_bot", "webappUrl": "…", "subscribers": 3 }
```

| Симптом | Причина и что делать |
|---|---|
| `enabled: false` | на этом сервере не задан `TELEGRAM_BOT_TOKEN` (или это старый деплой без кода бота — сверь, что собрана свежая ветка) |
| `enabled: true, running: false` | токен отвергнут Telegram или api.telegram.org был недоступен при старте — смотри `[telegram] …` в логах и перезапусти сервис |
| `running: true`, но `/start` молчит | тот же токен опрашивает второй процесс (`409 conflict` в логах) — оставь боту один сервер; либо сообщение старше 24 часов (Telegram хранит неподтверждённые апдейты сутки) |
| Кнопка → «сайт не позволяет установить соединение» | Telegram Web не смог встроить страницу: нужен свежий деплой с `frame-ancestors` (см. выше) и живой (не спящий) сервер |
| Кнопка открывается, но лента пустая | проверь, что `TELEGRAM_WEBAPP_URL` указывает на тот же сервер, где лежат треки |

### Нюансы

- **Один токен — один поллер.** Если два процесса одновременно опрашивают одного бота, Telegram отвечает `409`; бот логирует это и продолжает с паузами. Не запускай один токен в двух местах.
- **Render free-план засыпает** без входящего трафика — вместе с ним засыпает и бот (и API, и сайт). Платный план или внешний пинг раз в ~10 минут (UptimeRobot / cron-job.org на `/api/health`) решают это.
- **Встраивание в Telegram Web.** Сервер не отправляет `X-Frame-Options` и отдаёт `Content-Security-Policy: frame-ancestors https://*.telegram.org` — без этого Telegram Web (iframe) показывает «сайт не позволяет установить соединение». Нативные клиенты (iOS/Android/Desktop) используют webview и на эти заголовки не смотрят.
- Рассылка идёт в фоне и не тормозит загрузку трека артисту: ответ `201` уходит сразу, а сообщения отправляются со скоростью ~20/с (лимит Telegram — 30/с), заблокировавшие бот чаты отписываются на лету.
- Кнопки `web_app` требуют HTTPS: адрес по умолчанию — публичный Pages-бандл. Свой деплой указывай в `TELEGRAM_WEBAPP_URL`.

---

## Проверка

- `npm test` — 61 тест из двух сюит:
  - **44 на сервере** (`server/test/`): неверный/верный код, изоляция артистов по `artistKey`, публикация и валидация аудио, оценки (upsert, пересчёт, запрет артисту), удаление только владельцем, `410` для удалённого трека, восстановление гостя по коду + 8 тестов `shared/scoring.ts` (включая «90 из 90 только при единогласном максимуме» и совпадение округлений) + 16 тестов Telegram-бота (тексты и deep-links, хранилище подписчиков, связка «публикация → уведомление», живой прогон бота против мока Bot API: `/start`, рассылка, отписка при блокировке, заголовки frame-ancestors для Telegram Web, эндпоинт статуса);
